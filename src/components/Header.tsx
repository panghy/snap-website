...
interface HeaderProps {
  ...
  activePage?: 'catalogue' | 'specification' | 'home';
}

const Header: React.FC<HeaderProps> = ({ onCatalogueClick, onSpecificationClick, onHomeClick, isLightTheme, activePage = 'home' }) => {
      ...
      <header className={`header ${isLightTheme ? 'header-light' : ''}`}>
        ...
        <nav className={`nav-menu ${isMobileMenuOpen ? 'mobile-open' : ''}`}>
          <a
            ...
            onClick={(e) => {
              e.preventDefault();
              handleLinkClick(onCatalogueClick);
            }}
          >
            Browse SNAPs
          </a>
          <a
            href="#specification"
            className={`nav-link ${activePage === 'specification' ? 'active' : ''}`}
            onClick={(e) => {
              e.preventDefault();
              handleLinkClick(onSpecificationClick);
            }}
          >
            Specification
          </a>
          ...
          <a href="https://github.com/panghy/snap-website" target="_blank" rel="noopener noreferrer" className="nav-link github">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
